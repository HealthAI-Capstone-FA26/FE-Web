...
export const EncounterDetailModal: React.FC<EncounterDetailModalProps> = ({
  ...
  const getObservationValue = (code: string): string | number | undefined => {
    if (!latestVitalSession) return undefined;
    const obs = latestVitalSession.observations?.find(
      (o) => o.item?.itemCode?.toLowerCase() === code.toLowerCase()
    );
    return obs?.observationValue;
  };

  const hasVitals = !!latestVitalSession;
  const pulse = getObservationValue('PULSE') || getObservationValue('HR');
  const bpSys = getObservationValue('BP_SYS') || getObservationValue('BP_SYSTOLIC');
  const bpDia = getObservationValue('BP_DIA') || getObservationValue('BP_DIASTOLIC');
  const temp = getObservationValue('TEMP') || getObservationValue('TEMPERATURE');
  const spo2 = getObservationValue('SPO2');
  const resp = getObservationValue('RR') || getObservationValue('RESP');
